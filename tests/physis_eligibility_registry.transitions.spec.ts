...
describe("physis_eligibility_registry record transitions", () => {
  ...
  it("rejects suspending a revoked record", async () => {
	...
	const account =
	  await program.account.eligibilityRecord.fetch(fixture.eligibilityRecord);

	assert.strictEqual(account.status, RECORD_STATUS_REVOKED);
  });

  it("rejects suspending an expired record", async () => {
	const fixture = await createFixture(RECORD_STATUS_EXPIRED);

	await expectRejects(
	  () => suspendRecord(fixture),
	  "expired record cannot become suspended",
	);
  });
