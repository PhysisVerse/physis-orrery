...
use crate::events::EligibilityRecordRevoked;
...
pub struct RevokeEligibilityRecord<'info> {
    ...
    #[account(
        ...
        constraint = eligibility_record.registry == registry.key() @ EligibilityError::RecordRegistryMismatch,
        constraint = eligibility_record.eligibility_class == eligibility_class.key() @ EligibilityError::RecordClassMismatch,
        constraint = eligibility_record.class_id == class_id @ EligibilityError::InvalidClassId,
        constraint = eligibility_record.subject_kind == subject_kind
            && eligibility_record.subject_key == subject_key @ EligibilityError::InvalidSubjectKey
    )]
    pub eligibility_record: Box<Account<'info, EligibilityRecord>>,
}

pub fn process_revoke_eligibility_record(
    ...
    require!(
        ctx.accounts.eligibility_record.status != RECORD_STATUS_REVOKED,
        EligibilityError::EligibilityRecordAlreadyRevoked
    );
    ...
    eligibility_record.status = RECORD_STATUS_REVOKED;
